...
using Shader = FloraEngine.Rendering.Shaders.Shader;
...
namespace FloraEngine.Core
{
    public class ImGuiController : IDisposable
    {
        ...
        public ImGuiController(GL gl, IView view, IInputContext input, ImGuiFontConfig? imGuiFontConfig = null, Action? onConfigureIO = null)
        {
            ...
            {
                IntPtr glyphRange = imGuiFontConfig.Value.GetGlyphRange?.Invoke(io) ?? 0;
        ...
        private static void OnKeyDown(IKeyboard keyboard, Key keycode, int scancode) =>
            OnKeyEvent(keycode, scancode, down: true);

        private static void OnKeyUp(IKeyboard keyboard, Key keycode, int scancode) =>
            OnKeyEvent(keycode, scancode, down: false);
        ...
        // FIX: Made public so external resize handlers can call it directly
        ...
        private void SetPerFrameImGuiData(float deltaSeconds)
        {
            ...
            // FIX: Read live from _view every frame instead of relying on the
            // cached fields. This eliminates the one-frame stale window during
            // resize where _windowWidth/_windowHeight have not yet been updated
            // by WindowResized but the framebuffer has already changed.
            _windowWidth  = _view.Size.X;
            _windowHeight = _view.Size.Y;

            io.DisplaySize = new Vector2(_windowWidth, _windowHeight);

            if (_windowWidth > 0 && _windowHeight > 0)
            {
                // DisplayFramebufferScale tells ImGui how many physical pixels
                // correspond to one logical pixel. On a 2x HiDPI screen this
                // will be (2, 2). ImGui uses this when converting clip rects to
                // framebuffer coordinates inside RenderImDrawData.
                io.DisplayFramebufferScale = new Vector2(
                    (float)_view.FramebufferSize.X / _windowWidth,
        ...
        private void UpdateImGuiInput()
        {
            ...
            // FIX: Use the raw Vector2 position instead of casting to int via
            // Point. The int cast was truncating sub-pixel positions and also
            // the Point constructor path was redundant. More importantly,
            // mouseState.Position is already in logical (window) pixel space,
            // which matches io.DisplaySize — no DPI scaling needed here.
        ...
        private static ImGuiKey TranslateInputKeyToImGuiKey(Key key)
        {
            return key switch
            {
                ...
                Key.F13 => ImGuiKey.F13, Key.F14 => ImGuiKey.F14, Key.F15 => ImGuiKey.F15,
                Key.F16 => ImGuiKey.F16, Key.F17 => ImGuiKey.F17, Key.F18 => ImGuiKey.F18,
                Key.F19 => ImGuiKey.F19, Key.F20 => ImGuiKey.F20, Key.F21 => ImGuiKey.F21,
                Key.F22 => ImGuiKey.F22, Key.F23 => ImGuiKey.F23, Key.F24 => ImGuiKey.F24,
        ...
        private unsafe void SetupRenderState(ImDrawDataPtr drawDataPtr)
        {
            ...
            _shader.SetUniform("ProjMtx", orthoProjection);
...
#if !GLES && !LEGACY
            if (lastEnablePrimitiveRestart) _gl.Enable(GLEnum.PrimitiveRestart); else _gl.Disable(GLEnum.PrimitiveRestart);
        ...
        void main()
        {
            ...
            _shader = new FragVertShader(_gl, vertexSource, fragmentSource);
        ...
        private unsafe void RecreateFontDeviceTexture()
        {
            ...
            _fontTexture = new Texture2D(_gl, (void*) pixels, (uint) width, (uint) height, InternalFormat.Rgba8, PixelFormat.Bgra, PixelType.UnsignedByte, TextureUnit.Texture0);
            _fontTexture.SetDefaultParameters();
            _fontTexture.Bind();
